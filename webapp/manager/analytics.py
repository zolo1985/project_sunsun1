...
from webapp.manager.forms import SelectOption, SelectDriverOption, SelectSupplierOption
...
def manager_orders_stats():
    ...
    if form.validate_on_submit():
        ...
        if form.select_option.data == "Өдөр":
            ...
            return redirect(url_for('manager_analytics.manager_orders_stats'))

    return render_template('/manager/stats/stats.html', title='Хүргэлт стастистик', form=form)



@manager_analytics_blueprint.route('/manager/drivers-stats', methods=['GET','POST'])
@login_required
@has_role('manager')
...
@manager_analytics_blueprint.route('/manager/suppliers-stats', methods=['GET','POST'])
@login_required
@has_role('manager')
def manager_suppliers_stats():
    connection = Connection()
    ...
    if form.validate_on_submit():
        ...
        if form.select_option.data=="Өдөр":
            ...
            return render_template('/manager/stats/yearly_stats.html', data1=completed, data2=cancelled, data3=postphoned, data4=months, title='Хүргэлт стастистик', form=form, selected_date=form.date.data.month, selected_year=form.date.data.year, completed_rate=completed_rate, cancelled_rate=cancelled_rate, postphoned_rate=postphoned_rate)
        
    return render_template('/manager/stats/supplier_stats.html', title='Хүргэлт стастистик', form=form)